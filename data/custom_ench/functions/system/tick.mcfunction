...
# tag付与
    ...
    execute as @e[type=item] at @s if entity @a[distance=..10] if data entity @s {Item:{id:"minecraft:book"}} unless data entity @s Item.tag.StoredCustomEnchantments if block ~ ~-0.5 ~ enchanting_table run tag @s add CE.Book
    execute as @e[tag=CE.Book] at @s store result score $Lapis.Count CE.Temporary run clear @p lapis_lazuli 0
    execute as @e[tag=CE.Book] at @s run scoreboard players operation $XP.Level CE.Temporary = @p XP.Level
    execute as @e[tag=CE.Base] at @s run scoreboard players operation $XP.Level CE.Temporary = @p XP.Level

# 処理
    execute as @e[tag=CE.Base] at @s unless entity @p[gamemode=creative] if score $XP.Level CE.Temporary matches 5.. if entity @e[tag=CE.Add,distance=..1] run function custom_ench:core/main
